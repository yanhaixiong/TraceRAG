...
from src.postprocess.combine_multiple_question_report import category_report_generation
...
if __name__ == "__main__":
    ...
    for category_name, category_info in questions.items():
        for idx, retrieve_question in enumerate(category_info["questions"]):
            ...
            try:
                ...
                combined_file_path = os.path.join(output_dir, f'{question_name}_combined.txt')
                with open(combined_file_path, 'w', encoding='utf-8') as combined_f:
                    combined_f.write("\n\n".join(file_contents))

                print(f"\n\n[Done] Files processed for {question_name}, combined report saved.")

                try:
                    final_result = quesiton_report_generation(file_contents)
                    final_report_path = os.path.join(output_dir, f'{question_name}_report.txt')
    ...
    def collect_reports():
        result = []
        base_path =  config["conversation_directories"]["LLM_output"]
        for category in os.listdir(base_path):
            category_path = os.path.join(base_path, category)
            if os.path.isdir(category_path):
                section = f"reports related to {category.replace('_', ' ')}:"
                section_content = []

                for question in os.listdir(category_path):
                    question_path = os.path.join(category_path, question)
                    report_dir = os.path.join(question_path, "analyze")
                        ...
                        for file_name in os.listdir(report_dir):
                            if file_name.endswith("_report.txt"):
                                ...
                                with open(report_file_path, "r", encoding="utf-8") as f:
                                    ...
                                    section_content.append(f"{question}'s content : \"{content}\"")
                if section_content:
                    result.append(f"{section}\n" + "\n".join(section_content))

        return "\n\n".join(result)

    file_content = collect_reports()

    category_result = category_report_generation(file_content)

    final_report_save_path = os.path.join(config["conversation_directories"]["LLM_output"], "final_analysis_result.txt")
    with open(final_report_save_path, "w", encoding="utf-8") as f:
        f.write(category_result)
    print(f"final_analysis_result saved: output/LLM_output/final_analysis.txt")

    convert_txt_to_md_and_html(final_report_save_path)
